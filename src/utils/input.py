...
class Movement:
	def __init__(self, entity_manager, collisions=[], speed = 10, gravity = 3):
		...
		try:
			self.player_ground_collision_layer = self.collision[self.player.z_floor()-1]
		except IndexError:
	...
	def gravity(self, delta_time):
		if (self.player.x_floor(), self.player.y_floor()) not in self.player_ground_collision_layer:
	...
	def UP(self, delta_time):
		...
		if target not in self.player_collision_layer:
			...
			target_type = self.player_collision_layer[self.player_collision_layer.index(target) + 1]
